    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        okBtn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                DatabaseReference emptyRef = FirebaseDatabase.getInstance().getReference("users").child(user.getUid()).child("diary").child("empty");
                ...
                emptyRef.addChildEventListener(new ChildEventListener() {
                    @Override
                    public void onChildAdded(DataSnapshot dataSnapshot, String s) {
                        DiaryData empty = dataSnapshot.getValue(DiaryData.class);
                        Calendar temp = Calendar.getInstance();
                        temp.setTime(Utils.stringToDate(empty.getDate()));

                        if ((temp.before(mEndCal) || temp.equals(mEndCal)) && (temp.after(mStartCal) || temp.equals(mStartCal))) {
                            switch (empty.getEmotion()) {
                                case 0:
                                    if (happy_check.isChecked()) {
    ...
    public boolean onKeyDown(int keyCode,KeyEvent event){
        switch(keyCode){
            case android.view.KeyEvent.KEYCODE_BACK:
                /*번들 넘기기*/
                Intent intent = new Intent(SortingActivity.this, CalendarActivity.class);
